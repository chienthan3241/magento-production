...
class Product extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    ...
    const FEATURE_PRODUCT_CATEGORY_ID = 133;
...
//        $productIdsStr = false;
    ...
    /**
    * this function is created to return the position of products in Featured Category(133) to make sort by position in widget
    */
    public function getWidgetPosition($product) {
        $position = 0;
        $productId = $product->getId();
        $catergoryIds = $product->getCategoryIds();
        if (!in_array(self::FEATURE_PRODUCT_CATEGORY_ID, $catergoryIds)) {
            return $position;
        }
        $this->_resources = \Magento\Framework\App\ObjectManager::getInstance()->get('Magento\Framework\App\ResourceConnection');
        $connection = $this->_resources->getConnection();
        $sql = "select cp.* from catalog_category_entity cc join catalog_category_product cp on cc.entity_id = cp.category_id where cp.product_id = '$productId' and cc.entity_id = " . self::FEATURE_PRODUCT_CATEGORY_ID;
        $result = $connection->fetchAll($sql);
        if (isset($result[0]['position'])) {
