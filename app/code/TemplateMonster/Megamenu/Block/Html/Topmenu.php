...
class Topmenu extends \Magento\Theme\Block\Html\Topmenu
{
    ...
    public function renderMenu(
        ...
        foreach ($children as $child) {
            ...
            /**
            * Exception menu navigtion for landing pages
            */
            $buildUrl = '';
            $menuRedirect = parse_url($child->getUrl());
            if (isset($menuRedirect['path'])) {
                switch ($menuRedirect['path']) {
                    case '/uhren.html':
                        $menuRedirect['path'] = '/uhren-germantom';
                        $buildUrl = $this->_helper->unparse_url($menuRedirect);
                        break;
                    case '/kosmetik.html':
                        $menuRedirect['path'] = '/kosmetik-germantom';
                        $buildUrl = $this->_helper->unparse_url($menuRedirect);
                        break;
                    case '/taschen.html':
                        $menuRedirect['path'] = '/taschen-germantom';
                        $buildUrl = $this->_helper->unparse_url($menuRedirect);
                        break;
                    default:
                        $buildUrl = $child->getUrl();
                        break;
                }
            }
            /**
            ...
            */
            ...
            $html .= '<a href="' . $buildUrl . '" ' . $outermostClassCode . '>';
